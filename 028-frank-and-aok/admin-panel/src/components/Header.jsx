...
function Header() {
  ...
  useEffect(()=>{
    ...
    if(!cookiedata) Navigate('/');
  },[]);


  return (
    <div className={width === false ?
      ...
      <span className="my-[5px] text-[25px] w-[200px]  flex gap-[20px]">
        <IoMenuOutline className="cursor-pointer" onClick={() => {
      ...
      <span className="object-contain relative cursor-pointer"
        onClick={() => setProfileDropDown(!profileDropDown)}>
        <img src="/bgimg.jpg"
          alt="profile"
          width={40} height={45}
          className="rounded-[50%]" />
        <div className={profileDropDown === false ? "hidden" :
